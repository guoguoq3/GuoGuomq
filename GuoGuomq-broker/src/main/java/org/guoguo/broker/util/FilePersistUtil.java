    ...
    public void init() {
        try {
            ...
            if (!persistDir.exists()) {
                ...
                if (mkdirSuccess) {
                    log.info("GuoGuomq==============> 创建持久化目录成功 ：{}", persistDir.getAbsolutePath());
                } else {
                    log.info("GuoGuomq==============> 创建持久化目录失败{}", persistDir.getAbsolutePath());
...
//            ));
            ...
            /*
            关于前面已经找到最新的持久化文件了吗 为啥还要在恢复历史消息时传入目录currentPersistFile 只是 “当前正在写入的文件”，而目录中可能有多个历史文件（如 guoguomq-persist-1.log、guoguomq-persist-2.log 等）；
            ...
            log.error("GuoGuomq异常，初始化持久化目录失败======================>", e);
    ...
    public synchronized void writeMessage(String messageId, MqMessage message) {
       try {
           ...
           if(currentPersistFile.length()>=mqConfigProperties.getMaxFileSize()){
                log.info("GuoGuomq==============> 当前文件已超过最大大小，创建新的文件");
                //关闭流
               writer.close();
               //创建新文件
           ...
           //强制刷新 立即写入
           writer.flush();
           log.info("GuoGuomq 消息持久化成功：文件={}，消息ID={}", currentPersistFile.getName(), messageId);

       }catch (Exception e){
           log.error("GuoGuomq 消息持久化失败，消息ID={}", messageId, e);
           //todo：失败重试的机制
    ...
    private File findLatestPersistFile(File persistDir) {
        //获取persistDir目录下所有以"GuoGuomq-persist-"开头且以".log"结尾的文件列表
        File[] files = persistDir.listFiles((dir, name) -> name.startsWith("GuoGuomq-persist-") && name.endsWith(".log"));
        ...
        log.info("GuoGuomq==============> 找到最新的持久化文件：{}", latestFile.getAbsolutePath());
        return latestFile;
    }
    ...
    private File createNewPersistFile(File persistDir) {
        String fileName = "GuoGuomq-persist-" + System.currentTimeMillis() + ".log";
        File newFile = new File(persistDir, fileName);
        try {
            boolean createSuccess = newFile.createNewFile();
            if (createSuccess) {
                log.info("GuoGuomq==============> 新持久化文件创建成功：{}", newFile.getAbsolutePath());
                return newFile;
            } else {
                log.info("GuoGuomq==============> 新持久化文件创建失败：{}", newFile.getAbsolutePath());
                throw new IOException("新持久化文件创建失败");
            }
        } catch (Exception e) {
            log.error("GuoGuomq 新持久化文件创建失败", e);
    ...
    private void recoverMessagesFromFile(File persistDir) {
        File[] files = persistDir.listFiles((dir, name) -> name.startsWith("GuoGuomq-persist-") && name.endsWith(".log"));
        if (files == null || files.length == 0) {
            log.info("GuoGuomq 无历史持久化文件，无需恢复消息");
            return;
        }
        ...
        for (File file : files) {
            try (BufferedReader reader = new BufferedReader(new InputStreamReader(
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    if (parts.length != 2) {
                        log.warn("GuoGuomq 持久化文件解析异常，无效行：{}", line);
                        continue;
                    }
                  ...
                  try {
                      ...
                      log.error("GuoGuomq 文件内恢复消息序列化失败，文件：{} 消息ID: {}", file.getName(),messageId, e);
                  }


                }
                log.info("GuoGuomq 从文件 {} 恢复消息 {} 条", file.getName(), recoverCount);
            } catch (Exception e) {
                log.error("GuoGuomq 从文件 {} 恢复消息失败", file.getName(), e);
            }
        }

        log.info("GuoGuomq 消息恢复完成，共恢复 {} 条消息，失败 {} 条",
                recoverCount, recoverFailedIds.size());
        if (!recoverFailedIds.isEmpty()) {
            log.warn("GuoGuomq 恢复失败的消息ID：{}", recoverFailedIds);
        }
    }

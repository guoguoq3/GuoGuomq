...
public class MqBrokerHandler extends SimpleChannelInboundHandler<String> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, String msg) {
        ...
        if (cleanMsg.isEmpty()) {
            log.warn("GuoGuomq Broker 收到空消息");
            return;
        }
        ...
        try {
            ...
            log.error("GuoGuomq Broker 解析RPC消息失败: {}", cleanMsg, e);
            sendErrorResponse(ctx, "消息格式错误：" + e.getMessage());
            return;
        }
        if (rpcDto == null) {
            log.error("GuoGuomq Broker RPC消息解析结果为null: {}", cleanMsg);
            sendErrorResponse(ctx, "消息格式错误");
            return;
        }
        String methodType = rpcDto.getMethodType();
        if (methodType == null) {
            log.error("GuoGuomq Broker 方法类型为null: {}", rpcDto);
        ...
        try {
            ...
            log.error("GuoGuomq Broker 解析FunctionEntity失败: {}", rpcDto.getJson(), e);
            sendErrorResponse(ctx, "FunctionEntity格式错误：" + e.getMessage());
            return;
        }
        if (funcDto == null) {
            log.error("GuoGuomq Broker FunctionEntity解析结果为null: {}", rpcDto.getJson());

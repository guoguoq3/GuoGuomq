          ...
          //监听各处传来的消息
           if (MethodType.B_PUSH_MSG.equals(methodType)) {
               ...
               if (handleSuccess) {
                   ...
                   log.warn("GuoGuomq 消费者处理消息{}失败，发送FAIL ACK", messageId);
               }
               ...
               // 处理Broker的响应（如加入组成功、订阅成功）统一返回
                   ...
                   if (response.startsWith("ERROR")) {
                       ...
                       log.info("GuoGuomq 消费者收到Broker成功响应：{}", response);
                   }
           }

       }catch (Exception e){
           log.error("GuoGuomq 消费者处理消息异常", e);
       }
...
//    }
    ...
    public void channelInactive(ChannelHandlerContext ctx) {
        log.warn("GuoGuomq 消费者与Broker断开连接");
